...
#include "../../src/circular_buf.h"

// Create a test group
TEST_GROUP(circular_buf_test_group){
    void setup(){
...
TEST(circular_buf_test_group, setup_test) {
...
// Test null array
TEST(circular_buf_test_group, pull_empty_test) {
    circular_bufC buf;
    byte x = buf.pull();
...
TEST(circular_buf_test_group, push_test) {
    circular_bufC buf;
    buf.push(1);
...
TEST(circular_buf_test_group, pull_test) {
    circular_bufC buf;
    buf.push(1);
    byte x, z;
    for (x = 2; x <= 16; x++) {
